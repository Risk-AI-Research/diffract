"""Results erasing utility with kernel dependency resolution."""
...
class ResultsEraser:
    ...
    def erase(
        ...
        *,
        ...
        Args:
            view: Parameter view to erase scalar fields from.
        ...
        # Erase scalar fields from parameters (including contextual suffixes)
        re_patterns = [rf"{re.escape(f)}(@.+)?" for f in fields]
        view.erase_fields_with_regexp(*re_patterns)
        ...
        if aggregates is not None:
            ...
            if aggregates_to_erase:
                logger.debug(
                    "Erasing %d aggregate entries", len(aggregates_to_erase)
                )
