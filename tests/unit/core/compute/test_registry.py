...
class TestKernelConfig:
    ...
    def test_update_invalid_params(self):
        ...
        with pytest.raises(
            InvalidConfiguration, match="Invalid kernel configuration parameters"
        ):
...
class TestKernelRegistry:
    ...
    def test_configure_nonexistent_kernel(self, fresh_registry):
        ...
        with pytest.raises(
            DependencyNotFoundError, match="Kernel 'missing' not registered"
        ):
    ...
    def test_kernel_getters(self, fresh_registry, sample_kernel_func):
        ...
        assert (
            fresh_registry.get_kernel_apply_level("test_kernel")
            == KernelApplyLevel.IN_MODEL
        )
        assert (
            fresh_registry.get_kernel_execution_protocol("test_kernel")
            == KernelExecutionProtocol.PARALLEL
        )
        ...
        assert (
            fresh_registry.get_kernel_restrictions("test_kernel")
            == KernelRestrictions.BINARY
        )
    ...
    def test_resolve_dependencies_circular(self, fresh_registry):
        ...
        with pytest.raises(
            CircularDependencyError, match="Circular dependency for 'a'"
        ):
    ...
    def test_normalize_kernel_result_single_value(
        self, fresh_registry, sample_kernel_func
    ):
    ...
    def test_normalize_kernel_result_tuple_length_mismatch(
        self, fresh_registry, sample_kernel_func
    ):
        ...
        with pytest.raises(
            InvalidConfiguration, match="returned 3 values but produce declares 2"
        ):
            fresh_registry.normalize_kernel_result("test_kernel", (1, 2, 3))

    def test_normalize_kernel_result_scalar_multiple_fields(
        self, fresh_registry, sample_kernel_func
    ):
        ...
        with pytest.raises(
            InvalidConfiguration,
            match="returned scalar but produce declares multiple fields",
        ):
...
class TestIntegrationWithoutDI:
    ...
    def test_error_messages_quality(self, registry_with_mock_registry):
        ...
        with pytest.raises(
            DependencyNotFoundError, match=r"Kernel 'missing' not registered"
        ):
        ...
        with pytest.raises(
            CircularDependencyError, match=r"Circular dependency for 'a'"
        ):
    ...
    def test_large_graph_performance_and_cache(self, registry_with_mock_registry):
        ...
        for i in range(1, chain_length + 1):
            kernel_name = f"k{i}"
            kernel_decorator(require_fields=(prev_field,))(
                make_kernel_func(kernel_name)
            )
            prev_field = kernel_name

        # Resolve dependencies at the tail
        deps = registry.resolve_dependencies(f"k{chain_length}")
        assert deps[0] == "base"
        assert deps[-1] == f"k{chain_length}"
        assert len(deps) == chain_length + 1

        # Cache should contain the result
        assert f"k{chain_length}" in registry._resolve_cache
    ...
    def test_tuple_and_dict_result_mapping(self, registry_with_mock_registry):
        ...
        assert registry.normalize_kernel_result("ab_dict", {"a": 5, "b": 6}) == {
            "a": 5,
            "b": 6,
        }
    ...
    def test_kernel_info_defaults_and_usage(self, registry_with_mock_registry):
        ...
        assert info2.summary == "S"
        assert info2.notes == "N"
