...
def test_register_default_kernels_populates_fresh_registry(ram_container) -> None:
    ...
    effects), so that a bare wired container gets a fully populated registry
    independently of import order."""
    from diffract.core.compute.decorator import _DEFAULT_KERNEL_SPECS

    assert (
        "frob_norm" in ram_container.compute_singleton.kernel_registry().list_kernels()
    )
